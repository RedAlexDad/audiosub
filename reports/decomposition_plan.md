# План декомпозиции: audiosub

Автоматические субтитры в реальном времени для Ubuntu.
Захват звука ОС → распознавание речи (Vosk / Whisper.cpp) → вывод субтитров.

---

## 1. Инфраструктура проекта

| Задача | Описание |
|--------|----------|
| 1.1 | Инициализация Rust-проекта (Cargo.toml, workspace, lint/format настройки) |
| 1.2 | Настройка CI (GitHub Actions: сборка, clippy, test) |
| 1.3 | Конфигурация проекта (cli-аргументы + config файл + env) |
| 1.4 | Логирование (tracing) |

## 2. Захват аудио системы (Audio Capture)

**Суть:** Перехват системного аудиовыхода (то, что слышат колонки/наушники).

| Задача | Описание |
|--------|----------|
| 2.1 | **Исследование:** PulseAudio `monitor` vs PipeWire `loopback` — выбрать primary API |
| 2.2 | **Абстракция:** трейт `AudioCapture` с методом `fn stream() -> Flux<AudioChunk>` |
| 2.3 | **Реализация PulseAudio:** libpulse-sys / libpulse-binding — захват с `monitor` |
| 2.4 | **Реализация PipeWire:** pipewire-rs — как альтернативный backend |
| 2.5 | **ALSA fallback:** через `loopback` устройство |
| 2.6 | **Перекодировка:** захват в сыром формате → ресемплинг в 16kHz mono f32 (единый формат для всех ASR) |
| 2.7 | **Выбор устройства:** перечисление доступных источников/мониторов |

## 3. Движок распознавания речи (ASR Engine)

**Суть:** Абстрактный слой над Vosk / Whisper.cpp.

| Задача | Описание |
|--------|----------|
| 3.1 | **Абстракция:** трейт `AsrEngine { fn transcribe(chunk) -> Vec<Segment> }` |
| 3.2 | **Загрузка моделей:** авто-загрузка моделей по URL, кэширование в `~/.cache/audiosub/models` |
| 3.3 | **Backend: Vosk API:** `vosk-rs` — инициализация модели, инкрементальное распознавание |
| 3.4 | **Backend: Whisper.cpp:** запуск whisper.cpp через `whisper-rs`, batching |
| 3.5 | **Backend: Whisper.cpp (реальное время):** инкрементальный режим (слияние окон) |
| 3.6 | **Переключение моделей:** hot-reload или restart сессии при смене модели |
| 3.7 | **Язык:** аргумент `--lang` / автодетект языка |

## 4. Генерация субтитров (Subtitle Pipeline)

**Суть:** Сегменты распознанного текста → форматированные субтитры с таймкодами.

| Задача | Описание |
|--------|----------|
| 4.1 | **Сегментация:** сырые гипотезы → стабильные сегменты с временными метками |
| 4.2 | **Наложение таймкодов:** привязка текста к реальному времени системы |
| 4.3 | **Формат SRT:** вывод в стандартный `.srt` |
| 4.4 | **Формат VTT:** вывод в `.vtt` для веб |
| 4.5 | **Потоковый вывод:** запись в файл в реальном времени (append) |
| 4.6 | **Буферизация:** задержка для улучшения качества (показ с offset) |

## 5. CLI / TUI (User Interface)

**Суть:** Три режима — cli-only, интерактивный TUI, live overlay.

| Задача | Описание |
|--------|----------|
| 5.1 | **CLI (clap):** базовые аргументы: `--engine`, `--model`, `--output`, `--lang`, `--device` |
| 5.2 | **TUI (ratatui):** запуск с `--tui` — интерактивный режим |
| 5.3 | **Экран захвата:** индикатор уровня громкости (VU meter) |
| 5.4 | **Экран распознавания:** live-лента распознанного текста |
| 5.5 | **Экран субтитров:** превью текущих субтитров + управление |
| 5.6 | **Управление:** пауза/стоп/сброс/смена модели из TUI |
| 5.7 | **Логи:** встроенный просмотр логов в TUI |
| 5.8 | **Режим `--overlay`:** запуск оверлейного окна (GTK/wayland layer-shell) — stretch goal |

## 6. Обработка ошибок и надежность

| Задача | Описание |
|--------|----------|
| 6.1 | Graceful degradation при потере аудиопотока |
| 6.2 | Перезапуск ASR при падении процесса whisper.cpp |
| 6.3 | Восстановление после сбоев (recovery) |
| 6.4 | Логирование всех этапов (tracing + файл) |

## 7. Сборка и дистрибуция

| Задача | Описание |
|--------|----------|
| 7.1 | **Makefile:** единая точка входа: `make build/test/run/lint/docker` |
| 7.2 | **Dockerfile:** мультистейдж-сборка (builder + runtime на Ubuntu) |
| 7.3 | **Docker Compose:** `docker-compose.yml` с сервисами (app + dev) |
| 7.4 | Пакет для Ubuntu (`.deb`) |
| 7.5 | AppImage / статическая сборка |
| 7.6 | Документация: README, примеры, скриншоты |

---

## Приоритетная карта (MVP)

**Phase 1 — Ядро**
- 1.1, 1.2, 1.3, 1.4 — каркас проекта
- 2.1, 2.2, 2.3 — захват PulseAudio monitor
- 2.6 — ресемплинг
- 3.1, 3.3 — Vosk backend (первичный)
- 4.1, 4.2, 4.3, 4.5 — базовый SRT вывод
- 5.1, 5.2 — Basic CLI + TUI

**Phase 2 — Whisper.cpp**
- 3.4 — Whisper.cpp backend
- 3.6 — переключение движков

**Phase 3 — Полировка**
- 4.4 — VTT
- 5.3–5.7 — расширенный TUI
- 6.x — надежность

**Phase 4 — Дистрибуция**
- 7.1 — Makefile (сразу с фазы 1)
- 7.2, 7.3 — Docker + Compose
- 7.4–7.6 — упаковка, документация

---

## Стек технологий (предварительно)

| Компонент | Библиотека |
|-----------|------------|
| CLI аргументы | `clap` + `clap_complete` |
| TUI | `ratatui` + `crossterm` |
| Аудио захват | `libpulse-binding` / `pipewire-rs` |
| Ресемплинг | `rubato` |
| ASR Vosk | `vosk-rs` + Cargo feature |
| ASR Whisper | `whisper-rs` + Cargo feature |
| Асинхронность | `tokio` |
| Конфиг | `serde` + `toml` |
| Логи | `tracing` + `tracing-subscriber` |
| Загрузка моделей | `reqwest` |
| Сборка | `make` + `Docker` + `docker-compose` |
| CI/CD | `GitHub Actions` |
